...
- **Таблицы — парами stage/prod + селектор `IS_PROD` по `PROJECT_ROOT`** (решено 22-07-2026, §3.6; образец — брокер): dev-копия → stage-набор, prod-копия → prod-набор, одиночные `t__saas-gw-gc__*` новым кодом не используются. Идентификаторы пар — после выбора скоупа; судьба данных старых таблиц — открытый вопрос волны 1.
...
Полный трекаемый чек-лист — [`spec.md` §0.1](spec.md#01-волны-разработки-и-чек-лист-готовности); пункт попадает в волну по цене позднего решения (необратимое → до первой боевой записи, ломающее → до первого потребителя, аддитивное — сознательно позже). Ниже — только сводка.
