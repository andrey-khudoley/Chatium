...
Id Heap-таблицы глобален на аккаунт и **может быть объявлен ровно в одном файле** (повторное объявление — BuildError платформы `Detected duplicate heap table name`; подтверждено прогоном 22-07-2026). Поэтому каждый `tables/*.table.ts` объявляет **ровно одну таблицу — своего окружения**: в этой dev-копии — `__stage_`-id; перенос d/→p/ механически трансформирует сегмент id в `__prod_` (и пометки `(stage)`→`(prod)` в customMeta) — это второй трансформируемый класс файлов переноса после `config/routes.tsx`. Остальной код видит один объект (`BrokerModules`, `BrokerEvents`, …) и об окружениях не знает. Схемы окружений не разъезжаются, потому что файлы копий идентичны с точностью до сегмента id. Слой `repos/` в проекте отсутствует — `lib/` обращается к `tables/` напрямую (осознанное упрощение MVP).

Идентификаторы таблиц (Heap-имя `t__broker__<entity>__<env>_<suffix>`) фиксированы — суффикс общий для пары, различается только `__stage_`/`__prod_`.
