...
- **Чтение истории:** `readLogs` через `queryAi`; обязательны фильтр `workspace_path` (предположительно корень копии: `d/system/broker` stage / `p/system/broker` prod; подтвердить прогоном в каждом окружении — §0.1) и временные условия средствами SQL (`ts64` — `DateTime64`). `total` — отдельным `count()` для пагинации.
...
- **Ключи замков — массив `[LOCK_NS, назначение, …]`, `LOCK_NS = IS_PROD ? 'broker' : 'broker-stage'`** (§5.8): `[LOCK_NS, 'dedup', producerModuleKey, idempotencyKey]`, `[LOCK_NS, 'fanout', eventId]`, `[LOCK_NS, 'drainer']`. Массив, а не склейка через разделитель — иначе `a:b`+`c` и `a`+`b:c` дают один замок. Префикс — потому что пространство имён замков общее на аккаунт; сегмент окружения — потому что иначе stage- и prod-копии делят один замок (дренер одной молча вытесняет дренер другой).
