...
**Идентификаторы таблиц (§3, зафиксированы) — парами stage/prod:** `t__broker__<таблица>__stage_<хвост>` / `…__prod_<хвост>` (modules `wI7S9L`, events `BOnFpq`, deliveries `fk9ze2`, events-arch `LO6Zki`, settings `I7Ozm8`). Имя задаёт разработчик (платформа не генерирует; тип запрещает только двоеточие), формат — соглашение воркспейса: аккаунт платформа изолирует сама, проекты внутри аккаунта — нет; сегмент окружения разводит stage/prod-наборы. **Id объявляется ровно в одном файле аккаунта** (повторное объявление — BuildError платформы `Detected duplicate heap table name`, прогон 22-07-2026), поэтому каждая копия проекта объявляет **только таблицы своего окружения**: dev-копия `d/system/broker` — `__stage_`-id, prod-копия `p/system/broker` — `__prod_`-id; перенос d/→p/ трансформирует сегмент в `tables/*.table.ts` (второй трансформируемый класс файлов после `config/routes.tsx`). Все пять таблиц парные по именованию, общих нет (`008-heap.md`/`006-arch.md`). ⚠️ Неизменяемы: переименование молча уводит код на новую пустую таблицу.
...
- **Чтение истории:** `readLogs` через `queryAi`; обязательны фильтр `workspace_path` (= корень копии: stage `d/system/broker` — **подтверждено прогоном 22-07-2026**; prod `p/system/broker` — подтверждается лог-тестом при первом переносе, §0.2) и временные условия средствами SQL (`ts64` — `DateTime64`). `total` — отдельным `count()` для пагинации.
...
### Панель наблюдаемости (§5.11 · волна 2.5)

Одна админ-страница `web/admin` (`requireAccountRole 'Admin'`), три панели поверх готовых механизмов — ядро не меняется: **живой монитор логов** (подписка на `LOG_SOCKET_CHANNEL` + история через `readLogs`; payload записи — отдельным запросом, в живом потоке только при `Debug`), **статус-панель** (backlog fan-out, доставки по статусам, возраст старейшей `pending`, реестр модулей + кнопки `disable`/`enable` и переключатель `log_level`), **метрики** (Heap-агрегаты: события всего/за 24ч, разбивка по `eventType`, доля `dead`, активные модули; CH-зеркало — волна 3). API — `api/admin/*`, все `@shared-route` + `.run()` из Vue. Новых таблиц/полей/ключей настроек нет — волна целиком аддитивна (§0.1 «Волна 2.5»); приёмка — §9.5.2.5.
...
- **Синергия:** лог-тест волны 2 логирует из боевого кода и читает обратно — этим прогоном подтверждён `workspace_path` обоих окружений (§5.10.7, закрыто 22-07-2026).
...
Полный трекаемый чек-лист — [`spec.md` §0.2](spec.md#02-волны-разработки-и-чек-лист-готовности). Там же он разложен по **четырём волнам разработки** (прототип → MVP → beta → релиз): пункт попадает в волну по цене позднего решения — необратимое (форма строки, удаление данных) закрывается до первой боевой записи, ломающее (внешний контракт) — до первого внешнего потребителя, аддитивное откладывается сознательно. Ниже — только визуальная сводка.
...
flowchart TB
  ...
  subgraph TBL["Таблицы (Modules ✓, Events ✓, Deliveries ✓, EventsArchive ✓, Settings ✓)"]
    ...
    C2["retention/хранение — гибрид Heap-якорь + CH-зеркало (ADR-0011);<br/>контракт записи в CH-зеркало открыт (§0.2)"]
  end
```
