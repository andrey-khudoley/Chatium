...
**Идентификаторы таблиц (§3, зафиксированы):** `t__broker__modules__wI7S9L`, `…__events__BOnFpq`, `…__deliveries__fk9ze2`, `…__events-arch__LO6Zki`. Имя задаёт разработчик (платформа не генерирует; тип запрещает только двоеточие), формат `t__<проект>__<таблица>__<хвост>` — соглашение воркспейса: аккаунт платформа изолирует сама, проекты внутри аккаунта — нет. ⚠️ Неизменяемы: переименование молча уводит код на новую пустую таблицу.
...
- **Чтение истории:** `readLogs` через `queryAi`; обязательны фильтр `workspace_path` (предположительно `'p/system/broker'`, подтвердить прогоном — §0.1) и временные условия средствами SQL (`ts64` — `DateTime64`). `total` — отдельным `count()` для пагинации.
