# Spec-as-source: `p/template_metaproject/core`

Статус: источник истины для проекта.  
Последнее обновление: 2026-06-19.
Область действия: весь каталог `p/template_metaproject/core`.
...
`p/template_metaproject/core` - core-модуль сервиса Aley BPM, который хранит системные контракты, координирует взаимодействие модулей и выступает Heap-backed брокером событий.
...
Этот инвентарь является нормативным. Любой новый, удаленный или переименованный файл в `p/template_metaproject/core` требует синхронного изменения таблицы.
...
- `producerModule` и `consumerModule` задаются trusted server-side wrapper-ом как `moduleKey`, равный `PROJECT_ROOT` модуля из его `config/routes.tsx`, например `p/template_metaproject/core/sample-module`;
...
```ts
PROJECT_ROOT = 'p/template_metaproject/core'
ROUTES = {
  index: './',
  admin: './web/admin',
  profile: './web/profile',
...
- `getFullUrl('./')`, `getFullUrl('/')`, `getFullUrl('')` возвращают `/p/template_metaproject/core/`.
- `getFullUrl('./web/admin')`, `getFullUrl('/web/admin')`, `getFullUrl('web/admin')` возвращают `/p/template_metaproject/core/web/admin`.
- `withProjectRoot('./web/admin')` и `withProjectRoot('web/admin')` возвращают `./p/template_metaproject/core/web/admin`.
- `withProjectRoot('./')` и `withProjectRoot('')` возвращают `./p/template_metaproject/core/`.
- `withProjectRootAndSubroute('./web/admin', 'edit')` возвращает `./p/template_metaproject/core/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', '/edit')` возвращает `./p/template_metaproject/core/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', 'users/123')` возвращает `./p/template_metaproject/core/web/admin~users/123`.
...
- `projectName = BODY_TEXT`, сейчас `Template Metaproject`;
...
- добавить `<meta name="template-metaproject-page" content="web-tests" />`.
...
Heap table: `t__template-metaproject__setting__7Fk2Qw`.  
...
Heap table: `t__template-metaproject__log__9Xm3Kp`.  
...
Heap table: `t__template-metaproject__broker_module__3Mn7Qx`.
...
| Поле                    | Тип            | Требование                                                                                                                                               |
| ----------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `moduleKey`             | `Heap.String`  | Уникальный стабильный ключ модуля: значение `PROJECT_ROOT` из `config/routes.tsx`, например `p/template_metaproject/core/sample-module`, searchable off. |
...
Heap table: `t__template-metaproject__broker_event_contract__4Qr9Tx`.
...
Heap table: `t__template-metaproject__broker_subscription__6Vp4Ld`.
...
Heap table: `t__template-metaproject__broker_event__8Kf2Hn`.
...
Heap table: `t__template-metaproject__broker_delivery__5Dw9Rt`.
...
Heap table: `t__template-metaproject__broker_notification_attempt__2Ps8Na`.
...
Heap table: `t__template-metaproject__broker_ops_audit__7Rt8Lm`.
...
| ----------------------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------- |
| `project_name`                            | `Template Metaproject`       | На save приводится к string и `trim()`.                                                            |
| `project_title`                           | `Template Metaproject`       | На save приводится к string и `trim()`.                                                            |
...
- регистрирует module key `p/template_metaproject/core/sample-module`;
...
- вызывает broker только через `@app/app.runAppFunction` target `p/template_metaproject/core`;
...
| ------------- | -------------- | ----------------------------------------------------------------------------------------------- |
| `index`       | `/`            | `window.__BOOT__`, `Template Metaproject`                                                       |
...
| `web-tests`   | `/web/tests`   | `window.__BOOT__`, `template-metaproject-page`                                                  |
...
| `.dir.json`       | Метаданные каталога в workspace; текущее `name` равно `[INWORK] p/template_metaproject/core`.                                                                                                                                     |
