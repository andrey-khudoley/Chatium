# Spec-as-source: `p/template_metaproject`

Статус: источник истины для проекта.  
Последнее обновление: 2026-06-19.
Область действия: весь каталог `p/template_metaproject`.
...
`p/template_metaproject` - core-модуль сервиса Aley BPM, который хранит системные контракты, координирует взаимодействие модулей и выступает Heap-backed брокером событий.
...
Этот инвентарь является нормативным. Любой новый, удаленный или переименованный файл в `p/template_metaproject` требует синхронного изменения таблицы.
...
- `producerModule` и `consumerModule` задаются trusted server-side wrapper-ом как `moduleKey`, равный `PROJECT_ROOT` модуля из его `config/routes.tsx`, например `p/template_metaproject/sample-module`;
...
```ts
PROJECT_ROOT = 'p/template_metaproject'
ROUTES = {
  index: './',
  admin: './web/admin',
  profile: './web/profile',
...
- `getFullUrl('./')`, `getFullUrl('/')`, `getFullUrl('')` возвращают `/p/template_metaproject/`.
- `getFullUrl('./web/admin')`, `getFullUrl('/web/admin')`, `getFullUrl('web/admin')` возвращают `/p/template_metaproject/web/admin`.
- `withProjectRoot('./web/admin')` и `withProjectRoot('web/admin')` возвращают `./p/template_metaproject/web/admin`.
- `withProjectRoot('./')` и `withProjectRoot('')` возвращают `./p/template_metaproject/`.
- `withProjectRootAndSubroute('./web/admin', 'edit')` возвращает `./p/template_metaproject/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', '/edit')` возвращает `./p/template_metaproject/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', 'users/123')` возвращает `./p/template_metaproject/web/admin~users/123`.
...
| Поле                    | Тип            | Требование                                                                                                                                          |
| ----------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `moduleKey`             | `Heap.String`  | Уникальный стабильный ключ модуля: значение `PROJECT_ROOT` из `config/routes.tsx`, например `p/template_metaproject/sample-module`, searchable off. |
...
- регистрирует module key `p/template_metaproject/sample-module`;
...
- вызывает broker только через `@app/app.runAppFunction` target `p/template_metaproject`;
...
| `.dir.json`       | Метаданные каталога в workspace; текущее `name` равно `[INWORK] p/template_metaproject`.                                                                                                                                          |
