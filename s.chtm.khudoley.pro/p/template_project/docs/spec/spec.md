# Spec-as-source: `p/template_project`

Статус: источник истины для проекта.  
Последнее обновление: 2026-06-17.  
Область действия: весь каталог `p/template_project`.
...
Спецификация намеренно покрывает проект с избытком: кроме публичного поведения она фиксирует внутренние helper-ы, composable-слои, презентационные компоненты, тестовые раннеры и известные платформенные допущения. Это нужно, чтобы после копирования шаблона новый проект можно было поддерживать без обратного восстановления правил из кода.
...
`p/template_project` - базовый шаблон Chatium-проекта, который можно копировать в новый проект и развивать без добавления платформенных зависимостей.
...
- документацию, достаточную для копирования шаблона в новый проект.
...
Этот инвентарь является нормативным. Любой новый, удаленный или переименованный файл в `p/template_project` требует синхронного изменения таблицы.
...
- `getFullUrl('./')`, `getFullUrl('/')`, `getFullUrl('')` возвращают `/p/template_project/`.
- `getFullUrl('./web/admin')`, `getFullUrl('/web/admin')`, `getFullUrl('web/admin')` возвращают `/p/template_project/web/admin`.
- `withProjectRoot('./web/admin')` и `withProjectRoot('web/admin')` возвращают `./p/template_project/web/admin`.
- `withProjectRoot('./')` и `withProjectRoot('')` возвращают `./p/template_project/`.
- `withProjectRootAndSubroute('./web/admin', 'edit')` возвращает `./p/template_project/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', '/edit')` возвращает `./p/template_project/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', 'users/123')` возвращает `./p/template_project/web/admin~users/123`.
...
- добавить `<meta name="template-project-page" content="web-tests" />`.
...
Heap table: `t__template-project__setting__7Fk2Qw`.  
...
Heap table: `t__template-project__log__9Xm3Kp`.  
...
`getAdminLogsSocketId(ctx)` возвращает стабильный `admin-logs-a7f2b9c1`. Перед передачей на клиент он кодируется через `genSocketId`.
...
| `web-tests` | `/web/tests` | `window.__BOOT__`, `template-project-page` |
...
| `.dir.json` | Метаданные каталога в workspace; текущее `name` равно `[INWORK] p/template_project`; при копировании шаблона `name` должен соответствовать новому пути. |
...
## 15. Копирование шаблона

`p/template_project` является копируемым шаблоном, а не генератором. После копирования каталога в новое место новый каталог становится самостоятельным Chatium-проектом, и его путь от корня workspace становится runtime-идентичностью проекта.

Копирование выполняется так:

1. Скопировать весь каталог `p/template_project` в новый каталог, например `p/my_project`, `p/units/aayakovleva/sbp_client` или другой путь workspace.
2. Считать новый путь без ведущего `/` значением `PROJECT_ROOT` нового проекта.
3. Выбрать kebab-идентификатор для Heap/table/meta ключей, например `my-project` или `units-aayakovleva-sbp-client`.
4. До runtime-правок в скопированном проекте обновить его `docs/spec/spec.md`: заменить область действия, инвентарь, маршруты, table IDs, test IDs/описания и этот раздел так, чтобы новая спецификация стала источником истины именно для нового проекта.

Обязательные замены после копирования:

| Файл | Что заменить | Зачем |
| --- | --- | --- |
| `config/routes.tsx` | `export const PROJECT_ROOT = 'p/template_project'` на новый путь от корня workspace без ведущего `/`. | От этого зависят все ссылки, редиректы, `getFullUrl`, `withProjectRoot` и login back-url. |
| `.dir.json` | `name`, например на `"[INWORK] p/my_project"` или актуальное имя нового проекта. | Метаданные каталога для инструментов и навигации workspace. |
| `lib/tests/templateUnitRoutesChecks.ts` | Ожидаемое значение в проверке `routes_PROJECT_ROOT`: `PROJECT_ROOT === 'p/template_project'` на новый путь. | Иначе `GET /api/tests/unit` будет падать на проверке корня проекта. |
| `api/tests/list.ts` | Комментарий-шапку с `p/template_project`. | Чтобы справочный текст не указывал на исходный шаблон. |
| `api/logger/log.ts` | Пример message с `/p/template_project/lib`. | Чтобы пример логирования соответствовал новому проекту. |
| `web/tests/index.tsx` | `<meta name="template-project-page" content="web-tests" />` на уникальный meta marker нового проекта. | SSR-маркер используется HTTP-проверкой страницы тестов. Если marker меняется, синхронно обновляется `shared/testSuiteHelpers.ts` и раздел 12.3. |
| `shared/testSuiteHelpers.ts` | Ожидаемый HTTP fragment `template-project-page`, если изменен meta marker. | Чтобы HTTP-вкладка `/web/tests` проверяла новый SSR-маркер. |
| `tables/settings.table.ts` | Heap table ID `t__template-project__setting__7Fk2Qw`, если новый проект должен иметь отдельные настройки. | Иначе несколько копий в одном workspace будут делить настройки. |
| `tables/logs.table.ts` | Heap table ID `t__template-project__log__9Xm3Kp`, если новый проект должен иметь отдельные логи. | Иначе несколько копий в одном workspace будут писать в общий журнал. |
| `docs/data.md` | Описание table IDs, если table IDs изменены. | Legacy data reference должен совпадать с кодом. |
| `docs/spec/spec.md` | Все нормативные упоминания старого пути, table IDs, SSR marker, socket ID и инвентаря. | Спецификация нового проекта должна быть самостоятельным источником истины. |

Рекомендуемая механика замены:

- выполнить один глобальный поиск `template_project`, `p/template_project`, `template-project`, `t__template-project`, `template-project-page` в новом каталоге;
- заменить runtime-упоминания вне `docs/` на значения нового проекта;
- не делать слепую замену внутри `docs/` без ручной проверки, потому что спецификация может содержать примеры и исторические пояснения;
- после ручной проверки обновить docs так, чтобы они описывали уже новый проект, а не исходный шаблон.

Минимальная самопроверка по каталогу нового проекта:

```bash
rg -n "p/template_project|template_project|template-project|t__template-project|template-project-page|admin-logs-a7f2b9c1" <new-project-dir>
```

В runtime-коде нового проекта не должно остаться старых значений, кроме случаев, которые явно оставлены в новой спецификации. Если несколько копий проекта работают в одном workspace, уникальными должны быть как минимум `PROJECT_ROOT`, Heap table IDs и, при необходимости независимого live-канала логов, socket ID.

Настройки имени проекта после копирования:

- `config/project.tsx -> DEFAULT_PROJECT_TITLE` задает заголовок по умолчанию до сохранения настроек в Heap.
- `lib/settings.lib.ts -> DEFAULTS.project_name` и `DEFAULTS.project_title` задают дефолтные значения настроек.
- Эти значения можно заменить сразу под новый проект или оставить шаблонными и позже задать через админку.

Heap-изоляция:

- Если новый проект живет в том же workspace, что и исходный шаблон или другие копии, table IDs `Settings` и `Logs` должны быть заменены на уникальные ключи по соглашениям Heap.
- Если проект деплоится в отдельный workspace и общие таблицы невозможны, замена table IDs остается рекомендованной, но не обязательной для корректности.
- Любое изменение table ID требует синхронного обновления раздела 8, `docs/data.md` и проверок, которые зависят от настроек/логов.

Live log channel:

- `lib/logger.lib.ts -> LOG_SOCKET_HASH` сейчас формирует `admin-logs-a7f2b9c1`.
- Если несколько копий проекта должны иметь независимые admin log monitors в одном окружении, hash нужно заменить на уникальный.
- При изменении формата или конкретного ID обновляются раздел 10, unit-check `loggerLib_getAdminLogsSocketId_format` и связанные ожидания.

При добавлении собственных unit runner после копирования:

1. Создать отдельный `runXxxUnitChecks()` по образцу `runTemplateUnitChecks`, например в `lib/tests/xxxUnitSuite.ts`.
2. Использовать собственные `id` с доменным префиксом, например `lp_*`, `crm_*` или `pay_*`.
3. Добавить блок в `shared/testCatalog.ts -> UNIT_TEST_BLOCKS` с тем же множеством `id`.
4. В `api/tests/unit/index.ts` вызвать собственный runner до шаблонного runner.
5. Передать ids собственного runner в `runTemplateUnitChecks(extraRunnerIds)`, например через `xxxResults.results.map((r) => r.id)`.
6. Проверить `catalog_unit_ids_match_runner`: множество `id` в каталоге должно точно совпадать с множеством реально запускаемых unit tests.

Обязательные проверки после копирования и замен:

- `GET /api/tests/list` возвращает каталог нового проекта без старых вводящих в заблуждение описаний;
- `GET /api/tests/unit` завершается с `failed: 0`;
- `GET /api/tests/integration` под Admin завершается с `failed: 0` либо только с ожидаемыми environment-зависимыми строками, явно описанными в новой спецификации;
- `/`, `/web/login`, `/web/profile`, `/web/admin`, `/web/tests` открываются по новому `PROJECT_ROOT` с ожидаемыми ролями и redirect behavior;
- HTTP-вкладка страницы `/web/tests` проходит после обновления SSR marker;
- админка видит и сохраняет настройки нового проекта;
- live log monitor показывает события именно нового проекта, если socket/table IDs были изолированы.

Деплой после копирования:

- закоммитить изменения нового проекта;
- запушить ветку в репозиторий;
- дождаться автоматического деплоя Chatium;
- открыть приложение по новому пути, например `https://<домен>/p/my_project/`;
