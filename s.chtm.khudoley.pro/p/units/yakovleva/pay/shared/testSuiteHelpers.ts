// @shared
/**
 * Чистые помощники страницы тестов Payments Client: типы строк/секций, маппинг каталога
 * тестов в представление, метрики, проверки HTTP-страниц. Без состояния Vue —
 * используются composable useTestSuites и презентационными компонентами.
 */
...
export const HTTP_HTML_SNIPPETS: Record<string, string[]> = {
  ...
  'web-login': ['Вход'],
  /** Не «Юнит» из вкладок: в ответе может быть только head + boot, без полного тела до гидрации. */
