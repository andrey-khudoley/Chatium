# Документация — Payments Client (`p/units/yakovleva/pay`)

Индекс каталога `docs/`. Клиентская панель Payments Client (implementation-plan §1.8). Одно Chatium-приложение совмещает роли: **хранилище секретов магазина**, **серверная прокладка** `POST /api/gateways/invoke` между UI и payments-gateway (с журналом в Heap), **приёмники входящих webhook**, виджет-бандл для страницы оплаты GetCourse и **дашборд оператора** на `/`. Внутренние gateway roots и API-prefix задаются в `config/gatewayUrls.ts`; системные base URL-настройки являются optional override'ами.
...
| [api.md](api.md)                   | HTTP-эндпоинты: настройки (`api/settings/`), прокладка и аналитика (`api/gateways/*` под `guardInternalApi`), доступы (`api/access/*`, 6 эндпоинтов), webhook (`/webhooks/lifepay`), тесты.                                    |
...
- При обновлении `shared/redactRaw.ts` синхронизировать с gateway-версией этого shared-модуля.
