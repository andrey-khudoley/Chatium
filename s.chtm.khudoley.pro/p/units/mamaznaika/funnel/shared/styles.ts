/**
 * Дизайн-CSS сайта проекта «Мама Знайка — вебинарная воронка».
 * Инжектится строкой в <style> внутри index.tsx. Светлая «документная» тема
 * с красным акцентом (по образцу ipsen/kp). Классы с префиксом kp- — глобальные,
 * коллизий нет (единственная страница). Одноязычный сайт: переключателя языков нет.
 */
...
/* Заблокированная вкладка: приглушена, не кликается, с замком. */
.kp-tab.is-locked { color: var(--muted); cursor: not-allowed; opacity: .65; }
.kp-tab.is-locked:hover { background: transparent; color: var(--muted); }
.kp-tab-lock { margin-left: 5px; vertical-align: -2px; flex-shrink: 0; }
...
/* ── документные разделы (главный документ) ── */
...
.kp-ilist.inc li, .kp-ilist.exc li { padding-left: 28px; }
.kp-ilist.inc li::before { content: '+'; position: absolute; left: 4px; top: 0; color: var(--ok); font-weight: 800; }
.kp-ilist.exc li::before { content: '×'; position: absolute; left: 4px; top: 0; color: var(--accent); font-weight: 800; }
...
/* ── steps (вертикальный план выполнения: движение сверху вниз) ── */
.kp-steps { list-style: none; margin: 6px 0 0; padding: 0; }
.kp-step { position: relative; display: flex; gap: 14px; padding-bottom: 20px; }
.kp-step:last-child { padding-bottom: 0; }
/* Соединительная линия вниз от маркера к следующему шагу. */
.kp-step::before { content: ''; position: absolute; left: 13px; top: 28px; bottom: -2px; width: 2px; background: var(--line-strong); }
.kp-step:last-child::before { display: none; }
.kp-step.is-done::before { background: var(--ok); }
.kp-step-dot { position: relative; z-index: 1; flex-shrink: 0; width: 28px; height: 28px; border-radius: 50%; display: grid; place-items: center; background: var(--surface); border: 2px solid var(--line-strong); color: var(--muted); }
.kp-step.is-done .kp-step-dot { background: var(--ok); border-color: var(--ok); color: #fff; }
.kp-step.is-active .kp-step-dot { background: var(--surface); border-color: var(--accent); color: var(--accent); box-shadow: 0 0 0 4px var(--accent-soft); }
.kp-step.is-active .kp-step-dot::after { content: ''; width: 9px; height: 9px; border-radius: 50%; background: var(--accent); }
.kp-step-body { display: flex; flex-wrap: wrap; align-items: baseline; gap: 6px 12px; padding-top: 3px; }
.kp-step-text { font-weight: 600; color: var(--ink); }
.kp-step.is-done .kp-step-text { color: var(--ink-soft); }
.kp-step-status { font-size: 12.5px; font-weight: 600; letter-spacing: .01em; text-transform: uppercase; color: var(--muted); }
.kp-step.is-done .kp-step-status { color: var(--ok); }
.kp-step.is-active .kp-step-status { color: var(--accent); }
...
/* ═══════════════  ШАПКА: УЗКИЕ ЭКРАНЫ  ═══════════════
   Однорядной шапке (бренд + 4 вкладки) на узких экранах не хватает ширины —
   перестраиваем в два ряда, вкладки уходят в горизонтальную ленту. */
...
/* Если лента скроллится (очень узкие экраны) — подписи у краёв уходят в прозрачность. */
...
@media (max-width: 720px) {
  ...
  /* Короткие подписи вкладок: полные не помещаются на узком экране. */
  ...
  /* Бейджи уходят в свой ряд под текстом: иначе длинное имя автора съедает ширину. */
  ...
  /* Форма: кнопка на всю ширину. font-size 16px обязателен — иначе iOS Safari зумит. */
...
/* Ландшафт телефона: поджимаем липкую шапку и вертикальные отступы. */
