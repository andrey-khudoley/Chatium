...
import { CONTENT, ALL_QUESTION_IDS } from '../shared/content'
...
// Сайт одноязычный — контент берём напрямую (в отличие от мультиязычного ipsen).
const c = CONTENT
const ui = CONTENT.ui
...
// locked — вкладка под замком: не кликается, содержимое не открывается.
interface TabDef {
  id: string
  label: string
  short: string
...
const TABS: TabDef[] = [
  { id: 'doc', label: ui.tabDoc, short: ui.tabDocShort },
  { id: 'current', label: ui.tabCurrent, short: ui.tabCurrentShort },
  { id: 'target', label: ui.tabTarget, short: ui.tabTargetShort },
  { id: 'questions', label: ui.tabQuestions, short: ui.tabQuestionsShort, locked: true }
]
const activeTab = ref('doc')

/** Переключение вкладки: заблокированные (locked) не активируются. */
...
function badge(t: string): string {
  if (t === 'Real') return ui.badgeReal
  if (t === 'Bot') return ui.badgeBot
  return ui.badgeGuest
}

/** Подпись статуса шага плана выполнения (степпер раздела «План выполнения»). */
function stepStatusLabel(status: string): string {
  if (status === 'done') return ui.stepDone
  if (status === 'active') return ui.stepActive
  return ui.stepTodo
}

/**
 * Автор последнего ответа: имя, если есть, иначе — статус («гость» и т.п.).
 * Ответы отсортированы по createdAtMs asc, новые в конце — последний свежий.
 */
...
async function submit(qid: string) {
  ...
  try {
    ...
    if (res && res.success && res.answer) {
      ...
      f.ok = false
      f.msg = (res && res.error) || ui.errSend
    }
  } catch (e) {
    f.ok = false
    f.msg = ui.errNetwork
...
<template>
  <header class="kp-header">
    <div class="kp-shell kp-header-in">
      <div class="kp-brand">
        ...
        <span class="kp-brand-name">ИП Худолей Андрей · Мама Знайка</span>
      </div>
      <nav class="kp-tabs">
        <button
          v-for="t in TABS"
          :key="t.id"
          class="kp-tab"
          :class="{ 'is-active': activeTab === t.id, 'is-locked': t.locked }"
          :disabled="t.locked"
          :title="t.locked ? ui.tabLockedHint : undefined"
          @click="selectTab(t)"
        >
  ...
  <!-- ── ГЛАВНЫЙ ДОКУМЕНТ ── -->
  <main v-if="activeTab === 'doc'" class="kp-view">
    <div class="kp-shell">
      ...
      >
        ...
        <template v-else>
          <template v-for="(b, bi) in s.blocks" :key="bi">
            ...
            <ol v-else-if="b.type === 'steps'" class="kp-steps">
              <li
                v-for="(st, si) in b.steps || []"
                :key="si"
                class="kp-step"
                :class="'is-' + st.status"
              >
                <span class="kp-step-dot">
                  <svg
                    v-if="st.status === 'done'"
                    viewBox="0 0 24 24"
                    width="15"
                ...
                <div class="kp-step-body">
                  <span class="kp-step-text">{{ st.text }}</span>
                  <span class="kp-step-status">{{ stepStatusLabel(st.status) }}</span>
                </div>
              </li>
            </ol>
          </template>
  ...
  <!-- ── ТЕКУЩАЯ АРХИТЕКТУРА ── -->
  <main v-else-if="activeTab === 'current'" class="kp-view">
    <div class="kp-shell">
      <div class="kp-section-head">
        <h2>{{ ui.currentHeading }}</h2>
      </div>
      <div class="kp-lead">
        <p v-for="(t, i) in c.currentIntro" :key="i">{{ t }}</p>
      </div>

      <div v-for="layer in c.currentLayers" :key="layer.id" class="kp-layer">
  ...
  <main v-else-if="activeTab === 'target'" class="kp-view">
    <div class="kp-shell">
      <div class="kp-section-head">
        <h2>{{ ui.targetHeading }}</h2>
      </div>
      <div class="kp-lead">
        <p v-for="(t, i) in c.targetIntro" :key="i">{{ t }}</p>
      </div>

      <div v-for="layer in c.targetLayers" :key="layer.id" class="kp-layer">
        <div class="kp-layer-top">
          <span class="kp-layer-idx">{{ layer.index }}</span>
          <h3>{{ layer.title }}</h3>
        </div>
