...
/**
 * Корневой роут сайта проекта «Мама Знайка — вебинарная воронка».
 *
 ...
 * Весь статический контент (документ, архитектуры, вопросы) компонент берёт
 * сам из shared/content.ts. Сайт одноязычный (русский).
 */
export const indexPageRoute = app.html('/', async (ctx) => {
  ...
  return (
    <html lang="ru">
      <head>
        <title>Мама Знайка · {CONTENT.meta.title}</title>
        <meta charset="UTF-8" />
      ...
      <body>
        <SitePage answersByQuestion={answersByQuestion} />
      </body>
    </html>
  )
})
