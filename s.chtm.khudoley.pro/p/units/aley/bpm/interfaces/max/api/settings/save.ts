...
export const saveSettingRoute = app.post('/', async (ctx, req) => {
  ...
  await loggerLib.writeServerLog(ctx, {
    ...
    payload: {
      key,
      value: settingsLib.redactSettingValue(key, value),
  ...
  try {
    await loggerLib.writeServerLog(ctx, {
      ...
      payload: { key, value: settingsLib.redactSettingValue(key, value) }
    })
    await settingsLib.setSetting(ctx, key, value)
    const saved = await settingsLib.getSetting(ctx, key)
    const responseValue = settingsLib.redactSettingValue(key, saved)
    await loggerLib.writeServerLog(ctx, {
      severity: 6,
      message: `[${LOG_PATH}] Настройка сохранена`,
      payload: { key, value: responseValue }
    })
    await loggerLib.writeServerLog(ctx, {
      severity: 6,
      message: `[${LOG_PATH}] Возврат success`,
      payload: { key, saved: responseValue }
    })
