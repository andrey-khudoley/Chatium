...
BPM Core для Chatium. Минимальный старт: главная страница, админка, профиль и логин.
...
- Broker ops в админке: панель `components/admin/broker/BrokerOpsPanel.vue` показывает modules, subscriptions, events, deliveries, notifications через `api/admin/broker/diagnostics` и запускает audited Admin-операции toggle/requeue/skip/retry/raw-view через отдельные `api/admin/broker/*` routes. Raw payload не выдаётся в diagnostics; списочный summary строится только по явно разрешённым полям контракта. Raw payload открывается только через отдельное подтверждение с обязательным server-side reason.
...
- 2026-06-21: реализована админская broker ops панель (`components/admin/broker/*`) и подключена в `AdminPage.vue`; добавлены diagnostics/ops UI для modules, subscriptions, events, deliveries, notifications; `web/admin/index.tsx` теперь выполняет `requireAccountRole(ctx, 'Admin')` до логирования/SSR; обновлены `docs/api.md`, `docs/architecture.md`, `docs/imports.md`.
