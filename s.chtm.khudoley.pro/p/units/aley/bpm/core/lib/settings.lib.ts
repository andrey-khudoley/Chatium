...
export const SETTING_KEYS = {
  ...
  MAX_TEST_BOT_TOKEN: 'max_test_bot_token',
...
export const SECRET_SETTING_KEYS = [SETTING_KEYS.MAX_TEST_BOT_TOKEN] as const
...
export const DEFAULTS = {
  [SETTING_KEYS.PROJECT_NAME]: 'BPM Core',
  [SETTING_KEYS.PROJECT_TITLE]: 'BPM Core',
  ...
  [SETTING_KEYS.MAX_TEST_BOT_TOKEN]: '',
...
export async function setSetting(ctx: app.Ctx, key: string, value: unknown): Promise<void> {
  ...
  } else if (key === SETTING_KEYS.MAX_TEST_BOT_TOKEN) {
    normalized = typeof value === 'string' ? value.trim() : ''
    await loggerLib.writeServerLog(ctx, {
      severity: 6,
      message: `[${LOG_MODULE}] setSetting MAX_TEST_BOT_TOKEN branch`,
      payload: { key, value: redactSettingValue(key, normalized) }
    })
  } else if (
