...
`p/units/aley/bpm/core` - core-модуль сервиса Aley BPM, который хранит системные контракты, координирует взаимодействие модулей и выступает Heap-backed брокером событий.
...
Этот инвентарь является нормативным. Любой новый, удаленный или переименованный файл в `p/units/aley/bpm/core` требует синхронного изменения таблицы.
...
| `components/admin/broker/brokerOpsPanelModel.ts`       | Локальные типы, вкладки и статусы фильтров broker ops-панели.                                                               |
...
| `api/test-fixtures/max-init-data.ts`                   | `POST /api/test-fixtures/max-init-data`, генерация signed MAX `WebApp.initData` для тестов.                                 |
...
| `lib/broker/eventSummary.lib.ts`                       | Pure builder `BrokerEventSafe.primarySummary` только по module-owned display hints, без чтения произвольных payload fields. |
...
| `lib/testFixtures/maxInitData.lib.ts`                  | Server-only helper генерации valid/invalid MAX Mini App fixture payloads без раскрытия token.                               |
...
| `pagecss/adminBrokerOpsCss.ts`                         | Broker ops panel/table CSS for admin page.                                                                                  |
...
| `/api/test-fixtures/*` | Admin                  | Платформенная ошибка auth.                              |
...
- `producerModule` и `consumerModule` задаются trusted server-side wrapper-ом как `moduleKey`, равный `PROJECT_ROOT` модуля из его `config/routes.tsx`, например `p/units/aley/bpm/interfaces/max`;
...
```ts
PROJECT_ROOT = 'p/units/aley/bpm/core'
ROUTES = {
  index: './',
  admin: './web/admin',
  profile: './web/profile',
...
- `getFullUrl('./')`, `getFullUrl('/')`, `getFullUrl('')` возвращают `/p/units/aley/bpm/core/`.
- `getFullUrl('./web/admin')`, `getFullUrl('/web/admin')`, `getFullUrl('web/admin')` возвращают `/p/units/aley/bpm/core/web/admin`.
- `withProjectRoot('./web/admin')` и `withProjectRoot('web/admin')` возвращают `./p/units/aley/bpm/core/web/admin`.
- `withProjectRoot('./')` и `withProjectRoot('')` возвращают `./p/units/aley/bpm/core/`.
- `withProjectRootAndSubroute('./web/admin', 'edit')` возвращает `./p/units/aley/bpm/core/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', '/edit')` возвращает `./p/units/aley/bpm/core/web/admin~edit`.
- `withProjectRootAndSubroute('./web/admin', 'users/123')` возвращает `./p/units/aley/bpm/core/web/admin~users/123`.
...
```ts
DEFAULT_PROJECT_TITLE = 'BPM Core'
...
- `/web/admin` при отсутствии Admin возвращает HTML fallback с title `Вход`, clarity, meta refresh и `window.location.href` на project-root login URL через `getFullUrl(ROUTES.login)` + `URLSearchParams({ back: req.url })`; JS-строка URL экранируется через `JSON.stringify`.
...
- `projectName = BODY_TEXT`, сейчас `BPM Core`;
...
- добавить `<meta name="units-aley-bpm-core-page" content="web-tests" />`.
...
Heap table: `t__units-aley-bpm-core__setting__7Fk2Qw`.  
...
Heap table: `t__units-aley-bpm-core__log__9Xm3Kp`.  
...
Heap table: `t__units-aley-bpm-core__broker_module__3Mn7Qx`.
...
| Поле                    | Тип            | Требование                                                                                                                                     |
| ----------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `moduleKey`             | `Heap.String`  | Уникальный стабильный ключ модуля: значение `PROJECT_ROOT` из `config/routes.tsx`, например `p/units/aley/bpm/interfaces/max`, searchable off. |
...
Heap table: `t__units-aley-bpm-core__broker_event_contract__4Qr9Tx`.
...
| `eventType`           | `Heap.String` | Dot/kebab namespace события, например `max.message.received`.                                                          |
...
- `display.summaryFields` задается только module-owned manifest-ом владельца события в `contracts/brokerEvents.ts`; core не содержит event-specific эвристик и не знает MAX/другие payload-ы на уровне кода;
...
- если `display.summaryFields` отсутствует или после фильтрации не дал ни одного поля, `primarySummary` остается пустым; core не строит generic fallback по payload, чтобы diagnostics/list UI не раскрывал PII и доменные поля без явного module-owned display contract;
...
Heap table: `t__units-aley-bpm-core__broker_subscription__6Vp4Ld`.
...
Heap table: `t__units-aley-bpm-core__broker_event__8Kf2Hn`.
...
| `eventType`              | `Heap.String` | Dot/kebab namespace, например `max.message.created` или `miniapp.form.submitted`.                                  |
...
Heap table: `t__units-aley-bpm-core__broker_delivery__5Dw9Rt`.
...
Heap table: `t__units-aley-bpm-core__broker_notification_attempt__2Ps8Na`.
...
Heap table: `t__units-aley-bpm-core__broker_ops_audit__7Rt8Lm`.
...
| `reason`      | `Heap.String` | Причина действия; обязательна для mutating ops и raw payload view.                                                                        |
...
| ----------------------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------- |
| `project_name`                            | `BPM Core`                   | На save приводится к string и `trim()`.                                                            |
| `project_title`                           | `BPM Core`                   | На save приводится к string и `trim()`.                                                            |
...
| `max_test_bot_token`                      | `''`                         | Secret. Токен тестового MAX-бота для fixture/HMAC сценариев интерфейсных модулей.                  |
...
- `max_test_bot_token` используется только как источник тестовой fixture-секретки для MAX Bot/Mini App HMAC сценариев; raw token не попадает в SSR props, browser boot, logs, broker events или notification payload.
...
- `SETTING_KEYS` содержит ключи из таблицы выше, включая broker settings и `max_test_bot_token`.
- `SECRET_SETTING_KEYS` строго содержит `max_test_bot_token`.
...
- `getAllSettings(ctx)` возвращает объект defaults plus Heap rows, где row с `value !== undefined && value !== null` перекрывает default; для secret keys возвращается только `{ configured:boolean }`.
- `getRawSecretSettingString(ctx, 'max_test_bot_token')` является server-only helper-ом и возвращает raw string или `''`; его нельзя импортировать в Vue/shared-route/browser code.
...
- `setSetting(MAX_TEST_BOT_TOKEN)` принимает string, trim, хранит raw secret только server-side в Heap `Settings`; пустая строка очищает значение.
...
- `GET /api/settings/list` возвращает для `max_test_bot_token` только `{ configured:boolean }`, не raw value;
- `GET /api/settings/get?key=max_test_bot_token` возвращает `{ success:true, key, value:{ configured:boolean } }`;
- `POST /api/settings/save` принимает `{ key:'max_test_bot_token', value:string }`, сохраняет trim raw value в Heap `Settings`, но возвращает только `{ success:true, key, value:{ configured:boolean } }`;
- пустая строка для `max_test_bot_token` очищает secret;
- raw `max_test_bot_token` запрещено писать в `logs`, `ctx.account.log`, broker payload/metadata, notification payload, SSR props и browser boot.
...
- выполняет auth/upsert/contract registration внутри `runWithExclusiveLock(ctx, 'broker:module:' + moduleKey)`;
...
Fixture API обслуживает тестовые данные для интерфейсных модулей. Endpoint-ы Admin-only и не возвращают raw secrets.

| Method/path                             | Auth  | Request                     | Success response                                                   | Validation errors                                   |
| --------------------------------------- | ----- | --------------------------- | ------------------------------------------------------------------ | --------------------------------------------------- |
| `POST /api/test-fixtures/max-init-data` | Admin | `MaxInitDataFixtureRequest` | `{ success:true, initData, initDataUnsafe, tokenConfigured:true }` | Missing `max_test_bot_token`; invalid fixture body. |

`MaxInitDataFixtureRequest`:
...
`POST /api/test-fixtures/max-init-data`:

- читает raw `max_test_bot_token` только через server-only `getRawSecretSettingString`;
- генерирует URL-encoded `WebApp.initData` по тому же HMAC-SHA256 алгоритму, который MAX Mini Apps используют для server-side validation;
- `valid=false` возвращает intentionally invalid signature fixture для negative checks;
- `initDataUnsafe` в ответе является удобной тестовой расшифровкой и не считается доверенным runtime-источником;
- не возвращает raw token, token hash, intermediate HMAC keys или secret-derived debug fields;
- не пишет token в logs, `ctx.account.log`, Heap payload, broker event или notification payload.
...
| `POST /api/admin/broker/events/raw`           | Admin | `{ eventId, reason }`                          | `{ success:true, eventId, payload, metadata? }`                                       | `invalid_request`, `admin_reason_required`, `not_found`, `raw_payload_unavailable`                 |
...
- `BrokerEventSafe` включает `primarySummary`, построенный через `lib/broker/eventSummary.lib.ts` только по `BrokerEventContracts.display.summaryFields`, `eventType`, `eventVersion`, `contractKey`, `schemaHash`, но не включает raw `payload`;
...
- требует non-empty `reason`; пустой или отсутствующий reason возвращает `admin_reason_required`;
...
| ------------- | -------------- | ----------------------------------------------------------------------------------------------- |
| `index`       | `/`            | `window.__BOOT__`, `BPM Core`                                                                   |
...
| `web-tests`   | `/web/tests`   | `window.__BOOT__`, `units-aley-bpm-core-page`                                                   |
...
| `.dir.json`       | Метаданные каталога в workspace; текущее `name` равно `[INWORK] p/units/aley/bpm/core`.                                                                                                                                           |
...
- отдавать `max_test_bot_token` raw-значением в settings list/get responses, SSR props, browser boot, logs, broker event payload/metadata или notification payload;
