...
**Идентификаторы таблиц (§3, зафиксированы):** `t__aley-bpm-core__modules__wI7S9L`, `…__events__BOnFpq`, `…__deliveries__fk9ze2`, `…__events-arch__LO6Zki`. Имя задаёт разработчик (платформа не генерирует; тип запрещает только двоеточие), формат `t__<проект>__<таблица>__<хвост>` — соглашение воркспейса: аккаунт платформа изолирует сама, проекты внутри аккаунта — нет. ⚠️ Неизменяемы: переименование молча уводит код на новую пустую таблицу.
...
### Логирование (§3.6 / §5.10)

Единая точка — `writeServerLog`; прямые `ctx.account.log` в коде модуля запрещены (обходят отсечку и не попадают в Heap с админкой).

```mermaid
flowchart LR
  C["код брокера"] -->|"severity 0–7 + message + payload"| W["writeServerLog"]
  W --> G{"severity ≤ порога<br/>по настройке log_level?"}
  G -->|"нет"| X["отброшено"]
  G -->|"да"| P{"log_level = Debug?"}
  P -->|"нет"| M["только message"]
  P -->|"да"| MP["message + payload"]
  M --> O
  MP --> O
  O["пять выходов"] --> O1["ctx.log · stdout"]
  O --> O2["ctx.account.log · ClickHouse"]
  O --> O3["строка Logs · §3.6"]
  O --> O4["WebSocket · монитор админки"]
  O --> O5["вебхук · fire-and-forget"]
```

- **Шкала — syslog 0–7** (меньше = строже). Брокер использует `3` error, `4` warning, `6` info, `7` debug.
- **Настройка `log_level`** (`Disable`/`Error`/`Warn`/`Info`/`Debug`, дефолт `Info`) задаёт порог: проходит `0 ≤ severity ≤ порог`.
- **`payload` только на `Debug`** — на рабочем уровне видна карта вызовов, но не данные; заодно содержимое событий не оседает в журнале.
- **Рекурсия:** чтение настроек логгера логировать через `writeServerLog` нельзя.
- ⚠️ **У таблицы `Logs` нет retention** — пишется на каждый шаг, упрётся в лимит раньше доменных таблиц (§0.1).
