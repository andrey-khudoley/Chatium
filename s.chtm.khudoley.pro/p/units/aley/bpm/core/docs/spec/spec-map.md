...
Единая точка — `writeServerLog`; прямые `ctx.account.log` в коде брокера запрещены (обходят отсечку и не попадают в живой монитор). **Своей таблицы логов нет** — `ctx.account.log` уже пишет в ClickHouse `chatium_ai.account_logs`, Heap-копия была бы вторым экземпляром тех же данных.
...
flowchart LR
  ...
  O2 -.->|"чтение истории: queryAi"| Q[("@traffic/sdk<br/>⚠️ в этом workspace нет")]
```
...
- ⚠️ **Чтение истории заблокировано:** `queryAi` — единственный способ, `@traffic/sdk` в workspace отсутствует (§0.1). Остаются `/s/dev/logs` и живой монитор.
