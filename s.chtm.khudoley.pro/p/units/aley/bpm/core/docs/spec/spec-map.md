...
- **Чтение истории:** `readLogs` через `queryAi`; обязательны фильтр `workspace_path = 'p/units/aley/bpm/core'` и временные условия средствами SQL (`ts64` — `DateTime64`). `total` — отдельным `count()` для пагинации.
