...
import { runCheckoutUnitChecks } from './checkoutUnitSuite'
...
export function runTemplateUnitChecks(
  extraRunnerIds: readonly string[] = []
): TemplateUnitTestResult[] {
  ...
  // Checkout unit checks
  const checkoutResults = runCheckoutUnitChecks()
  for (const r of checkoutResults) results.push(r)
