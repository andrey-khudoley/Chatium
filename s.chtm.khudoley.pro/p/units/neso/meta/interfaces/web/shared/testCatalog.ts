...
export const UNIT_TEST_BLOCKS: TestCatalogBlock[] = [
  ...
  },
  {
    id: 'unit-checkout',
    title: 'checkout (unit)',
    description: 'normalizeForm, socketMessages, key builders, broker schema',
    tests: [
      { id: 'checkout_normalize_required_email', title: 'normalizeForm: email обязателен' },
      { id: 'checkout_normalize_required_amount', title: 'normalizeForm: amount обязателен' },
      {
        id: 'checkout_normalize_required_currency_fallback',
        title: 'normalizeForm: currency fallback'
      },
      { id: 'checkout_normalize_trim_email', title: 'normalizeForm: email тримируется' },
      {
        id: 'checkout_normalize_amount_string_to_number',
        title: 'normalizeForm: amount строка → число'
      },
      {
        id: 'checkout_normalize_amount_zero_invalid',
        title: 'normalizeForm: amount "0" невалиден'
      },
      {
        id: 'checkout_normalize_amount_abc_invalid',
        title: 'normalizeForm: amount "abc" невалиден'
      },
      {
        id: 'checkout_normalize_optional_empty_excluded',
        title: 'normalizeForm: пустые опц. поля исключаются'
      },
      {
        id: 'checkout_normalize_optional_present',
        title: 'normalizeForm: заполненные опц. поля включаются'
      },
      { id: 'checkout_normalize_non_object', title: 'normalizeForm: не объект → ошибка' },
      { id: 'checkout_msg_submitted_type', title: 'buildCheckoutSubmittedMessage: type' },
      { id: 'checkout_msg_payment_ready_type', title: 'buildPaymentReadyMessage: type + url' },
      {
        id: 'checkout_msg_payment_ready_optional',
        title: 'buildPaymentReadyMessage: без orderKey'
      },
      { id: 'checkout_msg_failed_type', title: 'buildCheckoutFailedMessage: type + error' },
      {
        id: 'checkout_buildIdempotencyKey_prefix',
        title: 'buildIdempotencyKey: префикс web-checkout:'
      },
      { id: 'checkout_buildSocketId_prefix', title: 'buildSocketId: префикс checkout:' },
      {
        id: 'checkout_buildEventIdempotencyKey_prefix',
        title: 'buildEventIdempotencyKey: префикс web-checkout-submitted:'
      },
      { id: 'checkout_key_builders_stable', title: 'key builders стабильны' },
      { id: 'checkout_broker_schema_format', title: 'broker schema: payloadSchemaFormat' },
      {
        id: 'checkout_broker_schema_no_additional',
        title: 'broker schema: additionalProperties false'
      },
      { id: 'checkout_broker_schema_required_5', title: 'broker schema: 5 required полей' },
      { id: 'checkout_broker_schema_required_keys', title: 'broker schema: required ключи верные' },
      {
        id: 'checkout_broker_schema_optional_fields',
        title: 'broker schema: опциональные поля в properties'
      },
      { id: 'checkout_broker_schema_no_ref', title: 'broker schema: нет $ref/format/minLength' }
    ]
  }
]
...
export const INTEGRATION_SERVER_TEST_BLOCKS: TestCatalogBlock[] = [
  ...
  },
  {
    ...
    description: 'Web checkout flow: submit, idempotency, process, orphan, status contract',
    tests: [
      { id: 'web_checkout_submit_publishes', title: 'submit публикует событие + создаёт строку' },
      {
        id: 'web_checkout_submit_idempotent',
        title: 'submit идемпотентен: два вызова — один publish'
      },
      {
        id: 'web_checkout_process_saves_payment',
        title: 'process: delivery → payment_ready + ack'
      },
      { id: 'web_checkout_orphan_not_acked', title: 'process: orphan delivery → fail, не ack' },
      { id: 'web_checkout_status_contract', title: 'checkoutStatusRoute: контракт ответа' }
    ]
  }
]
