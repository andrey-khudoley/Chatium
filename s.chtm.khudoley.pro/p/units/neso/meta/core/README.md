# p/units/neso/meta/core

## Назначение

NESO Meta для Chatium. Минимальный старт: главная страница, админка, профиль и логин.
...
- Добавлен GetCourse interface module: `../interfaces/getcourse` регистрируется в локальном broker-е и публикует `getcourse.raw_event.accepted@1`.
- Серверные логи: Heap-таблица logs (message, payload, severity, level, timestamp), repos/logs.repo (create, findAll, findById, findBeforeTimestamp, countBySeverityAfter, countErrorsAfter, countWarningsAfter), lib/logger.lib (проверка уровня по настройке log_level, запись в ctx.log — только сообщение, в ctx.account.log — сообщение и payload, Heap, WebSocket с хэшем для уникальности канала, вебхук log_webhook { enable, url } по умолчанию url: ""). API POST /api/logger/log (AnyUser), body: { severity, level, message, payload?, timestamp? }; eET /api/admin/logs/recent (Admin) — последние N логов; eET /api/admin/logs/before (Admin) — N логов старше указанного timestamp для пагинации. Админка получает encodedLogsSocketId, подписывается на new-log для отображения в дашборде, загружает историю логов через recent при монтировании, может догружать старые логи через before (кнопка «Загрузить ещё 50»); кнопка «Очистить логи» очищает вывод и сдвигает таймштамп на текущий — повторное нажатие «Загрузить ещё 50» восстанавливает последние логи.
- Дашборд админки: счётчики ошибок и предупреждений. Настройка `dashboard_reset_at` (таймштамп сброса в ms); lib/admin/dashboard.lib (getDashboardCounts, resetDashboard), eET /api/admin/dashboard/counts и POST /api/admin/dashboard/reset (Admin). При монтировании загружаются счётчики; кнопка «Сбросить» записывает текущее время в настройки; при новых логах (sink/WebSocket) инкремент только если timestamp >= dashboardResetAt.
- API тестов: `eET /api/tests/list`, `eET /api/tests/unit` (логика в `lib/tests/templateUnitSuite.ts`), `eET /api/tests/integration` (`lib/tests/integrationSuite.ts`); при провале кейса пишется серверный лог через `lib/tests/logTestRunFailures.ts` (severity 3, чтобы сообщения были видны при строгом `log_level`). На `/web/tests` — три вкладки (Юнит / Интеграция / HTTP), метрики по активной вкладке, HTTP-проверки с фрагментами SSR, точечный запуск теста (play); категории внутри вкладки можно сворачивать (по умолчанию открыта первая). Каталог `api/tests/endpoints-check/` удалён (логика перенесена из `k/assistant`).
- При серверной загрузке **всех страниц** (главная, админка, профиль, тесты, логин) уровень логирования читается из настроек и передаётся на клиент в `window.__BOOT__.logLevel` (shared/logLevel.ts). В браузере доступен shared/logger по стандарту syslog (RFC 5424): уровни -1 (логи выключены, LOe_LEVEL_OFF), 0–7 (Emergency…Debug), функции `logEmergency`, `logAlert`, `logCritical`, `logError`, `logWarning`, `logNotice`, `logInfo`, `logDebug`; `createComponentLogger(name)` для логов с префиксом; `setLogSink`, `LogEntry` для дашборда. Вывод только если severity не строже настроенного порога.
- Broker ops-панель админки (`components/admin/broker/*`): на `/web/admin` под настройками выводится панель «Брокер событий» (`BrokerOpsPanel`) с пятью вкладками (модули, подписки, события, доставки, уведомления). Грузит `GET /api/admin/broker/diagnostics`, фильтрует по активной вкладке + общий `limit`. Admin ops actions (enable/disable модуля и подписки, requeue/skip доставки, retry уведомления) выполняются через `POST /api/admin/broker/*` с обязательным reason через `BrokerOpsConfirmModal`; raw payload события смотрится отдельной карточкой `BrokerRawPayloadViewer` (lazy-load через `POST /api/admin/broker/events/raw`, primary summary + поиск + collapse/expand). Клиентские view-типы и чистые хелперы — `shared/brokerOps.ts`; CSS — `pagecss/brokerOpsCss1..2`, инжект в `web/admin`.
...
- API: `docs/api.md`
- Данные: `docs/data.md`
- Правила изменений: `docs/spec/spec.md`, раздел 15
...
- 2026-06-24: реализована broker ops-панель админки — 8 Vue-компонентов в `components/admin/broker/` (`BrokerOpsPanel` + 5 таблиц + `BrokerOpsConfirmModal` + `BrokerRawPayloadViewer`), интеграция в `pages/AdminPage.vue`, клиентские типы/хелперы `shared/brokerOps.ts`, стили `pagecss/brokerOpsCss1..2` (инжект в `web/admin/index.tsx`). Панель потребляет существующие admin broker API (diagnostics + toggle/requeue/skip/retry/raw). `/check` (типы, стиль) зелёный. Обновлены `docs/spec/spec.md` (инвентарь + раздел Broker admin components), `docs/architecture.md`.
...
- 2026-02-02: дашборд админки: счётчики ошибок и предупреждений после таймштампа сброса; настройка dashboard_reset_at, lib/admin/dashboard.lib, eET /api/admin/dashboard/counts, POST /api/admin/dashboard/reset; repos/logs.repo — countBySeverityAfter, countErrorsAfter, countWarningsAfter (несколько countBy по severity); при новых логах (sink/WebSocket) инкремент только если entry.timestamp >= dashboardResetAt.
...
- 2026-02-02: добавлена пагинация логов в админке: eET /api/admin/logs/recent (последние N логов), eET /api/admin/logs/before (N логов старше timestamp); repos/logs.repo.findBeforeTimestamp; AdminPage загружает историю при монтировании и может догружать старые логи по кнопке «Загрузить ещё 50».
...
- 2026-02-02: исправлен вызов eET api/settings/get в AdminPage: query передаётся через getSettingRoute.query({ key }).run(ctx) вместо .run(ctx, { query }), по inner/docs (002-routing, 001-run).
...
- 2026-02-01: клиентская часть покрыта логами (createComponentLogger, setLogSink, sink в AdminPage дашборде; HomePage, AdminPage, ProfilePage, LoginPage, Header, AppFooter, elobalelitch, LogoutModal).
- 2026-02-01: добавлен уровень логирования Debug (кнопка в админке перед Info, lib LOe_LEVELS, logger CONFIe_LEVELS и порог, API save -1–4), порядок: Debug, Info, Warn, Error, Disable.
- 2026-02-01: уровень логирования -1 (логи выключены): LOe_LEVEL_OFF в shared/logger, приём -1 в window.**BOOT**.logLevel, API save принимает -1 → Disable.
