# Документация — GetCourse Gateway (`p/gateways/getcourse`)

Индекс каталога `docs/`. Приложение Chatium — **gateway к API GetCourse**: публичные маршруты `/v1/{op}` (59 операций) + `GET /v1/operations`, рукописный каталог операций, проксирование на школу по заголовкам `X-Gc-School-Host` / `X-Gc-School-Api-Key`, панель мониторинга с raw-журналами и внутренней системой доступов. Каркас унаследован от `p/template_project`; источник правды по коду — дерево `p/gateways/getcourse`.
