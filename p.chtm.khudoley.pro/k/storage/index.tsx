...
import { PROJECT_TITLE, BODY_SUBTEXT } from './config/project'
...
export const indexPageRoute = app.html('/', async (ctx) => {
  ...
  const testsAiUrl = getFullUrl(ROUTES.testsAi)
  const serveExample = `${ctx.account.url(getFullUrl(ROUTES.serve))}?file=имя.js`

  return (
    <html>
      <head>
        <title>{PROJECT_TITLE} - Управление скриптами и стилями</title>
        <meta charset="UTF-8" />
        ...
        <style>{`
          ...
          @keyframes crt-power-on {
            ...
            8% { opacity: 0.2; transform: scale(0.97) translate(0.15px, -0.1px); filter: blur(1px); }
            16% { opacity: 0.4; transform: scale(0.98) translate(-0.12px, 0.08px); filter: blur(0.8px); }
            24% { opacity: 0.55; transform: scale(0.985) translate(0.1px, -0.06px); filter: blur(0.6px); }
            32% { opacity: 0.68; transform: scale(0.99) translate(-0.08px, 0.05px); filter: blur(0.5px); }
            40% { opacity: 0.78; transform: scale(0.995) translate(0.06px, -0.04px); filter: blur(0.4px); }
            48% { opacity: 0.85; transform: scale(0.998) translate(-0.04px, 0.03px); filter: blur(0.3px); }
            56% { opacity: 0.9; transform: scale(1.0) translate(0.03px, -0.02px); filter: blur(0.2px); }
            64% { opacity: 0.94; transform: scale(1.0) translate(-0.02px, 0.015px); filter: blur(0.15px); }
            72% { opacity: 0.97; transform: scale(1.0) translate(0.015px, -0.01px); filter: blur(0.1px); }
            80% { opacity: 0.99; transform: scale(1.0) translate(-0.01px, 0.008px); filter: blur(0.05px); }
            88% { opacity: 1; transform: scale(1.0) translate(0.008px, -0.005px); filter: blur(0.02px); }
            96% { opacity: 1; transform: scale(1.0) translate(-0.005px, 0.003px); filter: blur(0.01px); }
            100% { opacity: 1; transform: scale(1.0) translate(0, 0); filter: blur(0); }
          }
          #geometric-bg {
            position: fixed;
          ...
          @media (max-width: 768px) { body::after { border-radius: 0; } }
          body::before {
            content: '';
            position: fixed;
            top: 0; left: 0; right: 0; bottom: 0;
          ...
          @media (max-width: 768px) { body::before { border-radius: 0; box-shadow: inset 0 0 40px rgba(0, 0, 0, 0.2); } }
          #tv-glitch {
            position: fixed;
            top: 0; left: 0; right: 0; bottom: 0;
            z-index: 999997;
        ...
        <style>{`
          ...
          ::selection { background: #e0335a; color: #ffffff; }
          ::-moz-selection { background: #e0335a; color: #ffffff; }
      ...
      <body>
        ...
        <HomePage
          ...
          projectSubtitle={BODY_SUBTEXT}
          indexUrl={indexUrl}
          uiUrl={uiUrl}
          testsUrl={testsUrl}
          testsAiUrl={testsAiUrl}
