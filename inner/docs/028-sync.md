...
⚠️ **Первый элемент массива — держите уникальным на назначение** (наблюдение RV 22-07-2026, broker): при общем первом элементе у разных замков (`['broker-stage', 'fanout', id]` захватывается, пока держится `['broker-stage', 'drainer']`) захват деградирует до ~5-секундных поллов, и параллельные `runWithExclusiveLock` с тем же первым элементом падают по дефолтному таймауту 5с. Форма `['<ns>-<назначение>', …переменные]` (уникальный первый элемент) захватывается мгновенно. Механизм платформой не документирован — эмпирика; при проектировании составных ключей кладите назначение в первый элемент, а не отдельным элементом.
