...
- [Окружения stage/prod: два каталога проекта](#окружения-stageprod-два-каталога-проекта)
...
- ✅ Для окружений stage/prod — сегмент окружения в id таблицы, копия объявляет только своё (повтор id из второго файла — BuildError), см. [Окружения stage/prod: два каталога проекта](#окружения-stageprod-два-каталога-проекта)
...
## Окружения stage/prod: два каталога проекта

Паттерн для проекта, которому нужны раздельные данные для разработки и прода **в одном аккаунте**: две копии проекта (например, `d/project` — stage и `p/project` — prod), код переносится между ними копированием, данные разделены на уровне таблиц.

Основа — три факта:

1. **Идентичность Heap-таблицы — её строковый id, глобальный на аккаунт.** Но **объявлен id может быть ровно в одном файле аккаунта**: повторный `Heap.Table` с тем же id из другого файла (даже с идентичной схемой) — BuildError `Detected duplicate heap table name`; у «проигравшего» файла репозитории приходят `undefined` (прогон 22-07-2026, `008-heap.md`).
2. **Динамические имена таблиц невозможны** — id в `Heap.Table()` только литеральный, вычислить его в рантайме нельзя.
3. Поэтому окружение зашивается **в сегмент id**: каждый `.table.ts` объявляет ровно одну таблицу — своего окружения (`t__<project>__<entity>__stage_<хвост>` в `d/`-копии), а перенос кода `d/ → p/` **механически трансформирует** сегмент `__stage_` → `__prod_` в табличных файлах. Детали и код — `008-heap.md` → «Окружения stage/prod: сегмент окружения в id таблицы».

**Привязка «каталог = окружение»** — двумя механическими трансформациями переноса: `config/routes.tsx` (`PROJECT_ROOT`) и `tables/*.table.ts` (сегмент id). Для **не-табличных** аккаунт-глобальных имён служит селектор из `config/env.ts`:
...
Копирование `d/project` → `p/project` не требует ручного тумблера: сегмент id и `PROJECT_ROOT` привязывают копию к её набору таблиц и имён. Схемы окружений не разъезжаются, потому что файлы копий идентичны с точностью до сегмента id.

**Правила слоёв** (без них паттерн разваливается):

- Весь доступ к данным — через `repos/` (`HTTP → API → lib → repos → Heap`) либо, в малых проектах, `lib → tables` напрямую; в обоих случаях импортируется единственный экспорт `.table.ts`.
- Файл `.table.ts` объявляет и экспортирует **ровно одну** таблицу — своего окружения; второй id этой сущности в кодовой базе копии не встречается.
- Общая для окружений таблица напрямую невозможна (id объявляется один раз): если данные должны быть общими — таблицу объявляет один выделенный проект, остальные ходят через его API/`app.function`.
- Изменения схемы: аддитивные (добавить поле) — безопасны; переименования/удаления полей — только синхронно с копированием кода между каталогами (схема привязана к id глобально).
- **Таблицы — не единственная аккаунт-глобальная идентичность.** Ключи эксклюзивных замков (`runWithExclusiveLock`/`tryRunWithExclusiveLock`, `028-sync.md`), имена WebSocket-каналов (`sendDataToSocket`), имена custom job queue и прочие строковые ключи, видимые всему аккаунту, у копий проекта совпадут: stage и prod начнут вытеснять друг друга — джоб одной копии молча пропускает проход под чужим замком, в живой монитор льются чужие записи. Всё, что глобально на аккаунт, скоупится **тем же селектором** — сегментом окружения в ключе/имени.
...
- **008-heap.md** — База данных Heap; пары таблиц stage/prod и селектор окружения

---

**Версия**: 1.2  
**Дата**: 2025-11-02  
**Последнее обновление**: 2026-07-22 (правило «таблицы — не единственная аккаунт-глобальная идентичность»: ключи замков, имена WS-каналов и очередей у stage/prod-копий скоупятся тем же селектором окружения)
