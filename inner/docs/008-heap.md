...
5. **`limit` — предохранитель, а не размер партии** (см. [deleteAll](#deleteall---массовое-удаление)). ⚠️ Поведение при превышении по аналогии с `deleteAll`; отдельным прогоном для `updateAll` **не подтверждено**.
