...
- [Создание таблицы](#создание-таблицы)
  ...
  - [Окружения stage/prod: пары таблиц и селектор](#окружения-stageprod-пары-таблиц-и-селектор)
...
### Окружения stage/prod: пары таблиц и селектор
...
Для разделения окружений (stage/prod) это и не нужно. Ключевой факт: **идентичность таблицы — её строковый id, глобальный на аккаунт**. Файл и каталог, где объявлена таблица, значения не имеют: две копии проекта, объявляющие таблицу с одним id, работают с одной физической таблицей.

**Паттерн — пары таблиц + статический селектор.** Один объект полей, два вызова `Heap.Table` с литеральными id, наружу экспортируется только выбранный репозиторий:
...
// Один объект полей на пару — схемы не разъезжаются, типы идентичны
const orderFields = {
  userId: Heap.Optional(Heap.String({ customMeta: { title: 'Пользователь' } }))
  // ... остальные поля — ровно один раз
}

const OrdersStage = Heap.Table('t__project__orders__stage_Ab1Cd2', orderFields, {
  customMeta: { title: 'Заказы (stage)' }
})
const OrdersProd = Heap.Table('t__project__orders__prod_Ab1Cd2', orderFields, {
  customMeta: { title: 'Заказы (prod)' }
})

// Весь остальной код импортирует ЭТО и про окружения не знает
export const Orders = IS_PROD ? OrdersProd : OrdersStage
```

Оба вызова `Heap.Table` — с литеральными id, всё в рамках платформы. «Динамика» — только выбор между двумя готовыми репо-объектами, это чистый JS. Типы пары идентичны (один объект полей), поэтому для TypeScript и кода ниже разницы нет.

**Селектор** — статический, из конфига проекта: выводится из `PROJECT_ROOT`, который и так отличается между копиями проекта (схема двух каталогов — см. `006-arch.md` → «Окружения stage/prod: два каталога проекта»):
...
**Общие таблицы** (например, настройки) объявляются без пары — одинаковый id в обеих копиях проекта = одна физическая таблица.

**Каверзы**:

- **Обе копии объявляют оба id.** Схема привязана к id глобально. Аддитивные изменения (добавить поле) безопасны; переименования и удаления полей — только синхронно с копированием кода между окружениями.
...
- **Экспортируйте только выбранный репозиторий** (`Orders`); пару `OrdersStage`/`OrdersProd` наружу не отдавайте — прямой импорт в обход селектора должен быть невозможен.
- **Рантайм-переключатель** (селектор из настройки в общей таблице, асинхронная репо-фабрика) возможен, но не рекомендуется: лишнее чтение на каждый запрос, риск переключения посреди запроса и, главное, dev-код получает возможность случайно писать в prod-таблицы.
...
✅ **Для окружений stage/prod — пары таблиц + статический селектор** (динамические имена таблиц невозможны): см. [Окружения stage/prod: пары таблиц и селектор](#окружения-stageprod-пары-таблиц-и-селектор).
