...
- [Практические примеры](#практические-примеры)
  ...
  - [Рекуррентные платежи](#реккурентные-платежи)
...
### ⚠️ Лимит времени выполнения

Рантайм ограничен по времени (платформенное ограничение):

- **Джоб (`app.job`)** — до **60 секунд** на один запуск.
- **Вызов через эндпоинт** (роут/`app.function`) — до **20 секунд**.

Дольше — задачу дробят: обрабатывают данные партиями (учитывая лимит выборки **1000 строк на запрос**, см. `008-heap.md`) и/или распараллеливают на несколько независимых джобов (`scheduleJobAsap` по слайсам), либо джоб самоперепланируется и догоняет остаток на следующем запуске. Оценивая объём за один запуск, закладывайте, что 60 секунд делятся между чтением партий, операциями в рантайме и записью (в т.ч. выгрузкой файла).

> ⚠️ **Превышение бюджета обрывает джоб после текущей операции — код за ней не выполнится** (проверено прогоном 22-07-2026, см. `008-heap.md`, раздел про `deleteAll`). Отдельная операция (`deleteAll`, тяжёлая запись) при этом дорабатывает до конца в фоне, но обработчик не возобновляется, и всё, что стояло после неё, теряется. **Практическое следствие для самоперепланирования: `scheduleJobAt(следующий_запуск)` нельзя ставить в конце джоба, если до него идёт работа, способная перерасти бюджет** — переросший проход не перепланируется, и цепочка встанет навсегда. Перепланируйте **в начале** обработчика (до тяжёлой части) либо ведите запуск внешним планировщиком (cron), а не самоперепланированием. По той же причине «job success» у переросшего джоба платформа не фиксирует — судить о завершении можно только по собственному логу, записанному **до** тяжёлого шага.
...
job.scheduleJobAfter(
  ...
  unit:
    ...
    | 'years',
  params: any
): Promise<number>  // Возвращает taskId (число), требует await
```
...
```typescript
// Через 30 секунд
const taskId = await reminderJob.scheduleJobAfter(ctx, 30, 'seconds', {
  userId: ctx.user.id,
  message: 'Напоминание через 30 секунд'
})

// Через 9 минут
const taskId = await reminderJob.scheduleJobAfter(ctx, 9, 'minutes', {
  userId: ctx.user.id,
  message: 'Напоминание через 9 минут'
})

// Через 2 часа
const taskId = await reminderJob.scheduleJobAfter(ctx, 2, 'hours', {
  userId: ctx.user.id,
  message: 'Напоминание через 2 часа'
})

// Через 7 дней
const taskId = await reminderJob.scheduleJobAfter(ctx, 7, 'days', {
  userId: ctx.user.id,
...
```typescript
// 500 миллисекунд
await job.scheduleJobAfter(ctx, 500, 'milliseconds', params)

// 30 секунд
await job.scheduleJobAfter(ctx, 30, 'seconds', params)

// 5 минут
await job.scheduleJobAfter(ctx, 5, 'minutes', params)

// 2 часа
await job.scheduleJobAfter(ctx, 2, 'hours', params)

// 1 неделя
await job.scheduleJobAfter(ctx, 7, 'days', params)

// 1 месяц (приблизительно)
await job.scheduleJobAfter(ctx, 30, 'days', params)

// 1 неделя
await job.scheduleJobAfter(ctx, 1, 'weeks', params)

// 1 квартал
await job.scheduleJobAfter(ctx, 1, 'quarters', params)

// 1 год
await job.scheduleJobAfter(ctx, 1, 'years', params)
```

### Важно про типы возврата

- Для `jobRef.scheduleJobAfter/scheduleJobAsap/scheduleJobAt` (через `app.job(...)`) в текущих типах платформы возвращается `Promise<number>`.
- В `@app/jobs` есть deprecated глобальные функции `scheduleJobAfter/scheduleJobAsap/scheduleJobAt` с другим интерфейсом (включают `url`) и `Promise<string>`.
- В этом документе используется и рекомендуется современный вариант: методы экземпляра задачи через `app.job(...)`.
...
export const startCleanupRoute = app.post('/start-cleanup', async (ctx) => {
  await cleanupOldDataJob.scheduleJobAsap(ctx, {
    tableName: 'logs',
    daysOld: 30,
    recurring: true
  })
...
⚠️ **Не заменяйте задержку активным ожиданием.** Цикл вида `while (Date.now() - start < ms) {}` формально «работает», но **блокирует event loop**: асинхронные операции платформы в это время не выполняются. Проверено на прототипе логирования — busy-wait не давал уже сделанной записи `ctx.account.log` дойти до ClickHouse, и запись выглядела потерянной. Если нужна пауза, это всегда `scheduleJobAfter`, а не ожидание в коде.
