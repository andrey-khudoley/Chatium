...
Проверено прогоном 21-07-2026 на `chatium_ai.account_logs` (671 128 строк).

**Единственный жёсткий предел — размер ответа: ~20 МБ сырого JSON.** Ограничения на *число* строк нет; упирается именно объём.

| `LIMIT` | Узкая выборка (`ts64, level, msg`) | Длительность |
|---|---|---|
| 100 | 100 строк | 477 мс |
| 1 000 | 1 000 | 233 мс |
| 10 000 | 10 000 | 338 мс |
| 50 000 | 50 000 | 875 мс |
| 100 000 | 100 000 (≈10 МБ) | 1 126 мс |
| 500 000 | ❌ ошибка: ответ 49.7 МБ > лимит 20 МБ | 1 396 мс |

Отсюда практическое правило: **потолок считается в мегабайтах, а не в строках.** Узкая выборка (~105 байт/строка) помещается тысяч на сто; выборка с широкими колонками (`kv`, `json_str`) — во столько раз меньше, во сколько строка толще. ⚠️ Верхняя граница для широких выборок **не измерена**: в прогоне `kv`/`json_str` у попавших записей оказались пустыми, так что 50 000 «широких» строк прошли только формально.

Для страницы с пагинацией это означает, что `LIMIT` можно ставить любой разумный (сотни, тысячи), а думать надо про ширину `SELECT` — не тянуть `json_str`, если он не нужен для отображения.

**Сложность запроса не ограничена.** Прошли все проверенные конструкции:

| Конструкция | Длительность |
|---|---|
| `GROUP BY` с агрегатами | 365 мс |
| Подзапрос `SELECT count() FROM (SELECT … LIMIT 100)` | 388 мс |
| `SELF JOIN … ON level` | 510 мс |
| Оконная функция `row_number() OVER (ORDER BY ts64)` | 429 мс |
| `SELECT count()` без `WHERE` (полный скан 671K) | 363 мс |

**Собственного таймаута не обнаружено** — ни один запрос не оборвался по времени, включая полный скан.
